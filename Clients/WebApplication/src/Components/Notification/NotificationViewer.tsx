...
const NotificationViewer = ({notif}:Props) => {
    ...
    const [formatted , setFormatted] = useState< {mentions: string[]; parts: string[] ; regex:RegExp}>()

    useEffect(()=>{
        setFormatted(FindMentions(notif.content))
    },[])
    ...
    return (
        <div className=" p-5 pb-2 border-y text-lg border-darkGray">
            <div className="flex gap-3 items-center">
                ...
                {!notif.isSeen && <span className="indicator-item indicator-middle indicator-start badge-sm rounded-full badge-primary"></span>}
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-8">
                    <path fillRule="evenodd" d="M5.25 9a6.75 6.75 0 0 1 13.5 0v.75c0 2.123.8 4.057 2.118 5.52a.75.75 0 0 1-.297 1.206c-1.544.57-3.16.99-4.831 1.243a3.75 3.75 0 1 1-7.48 0 24.585 24.585 0 0 1-4.831-1.244.75.75 0 0 1-.298-1.205A8.217 8.217 0 0 0 5.25 9.75V9Zm4.502 8.9a2.25 2.25 0 1 0 4.496 0 25.057 25.057 0 0 1-4.496 0Z" clipRule="evenodd" />
                </svg>
                {/* <div className="bg-base-300 grid h-32 w-32 place-items-center"></div> */}
                </div>

                {formatted?.parts.map((part, index) =>
                    formatted.regex.test(part) ? (
                        <Link to="" key={index} className="text-primary">
                        {part}
                        </Link>
                    ) : (
                        part
